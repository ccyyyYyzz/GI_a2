...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \begin{minipage}{0.92\textwidth}
      ...
      makes a scene subspace exactly invisible under an idealized band-limited model;
      on an unfiltered bench that wall becomes a calibrated $\sim\!7\times10^{-3}$
      residual, suppressed by near-conjugate relay imaging
      (Section~\ref{sec:transfer}). We turn that blindness into a calibrated,
      certifiable design variable. A physics mapping assigns each optical symmetry
      class its wall type---a support wall and a lossless-bucket energy wall, exact to
      machine precision in the model (leaks near $10^{-16}$)---with the one controlled
      breaker that reopens it. A measured certification apparatus turns
...
walls of distinct physical type realize it exactly \emph{in this idealized
band-limited model}; on an unfiltered bench each becomes the calibrated
$\sim\!7\times10^{-3}$ residual of Section~\ref{sec:transfer}, suppressed by
near-conjugate relay imaging.
...
$2.18\times10^{-16}$ at every order and for every $(\eps,z_2)$---the phase-only
diagonal subgroup, the only orbit measured here. Within the model assumptions the
energy bucket is blind to any unitary spatial-mode transformation; the larger
\emph{mode-unitary} wall and its parity, global-phase, and polarization-$SU(2)$
siblings (Supplement~S1, Table~S1) are corollaries of Theorem~\ref{thm:noether} that
we present as falsifiable predictions of the framework, measured here only for the
phase-only subgroup.
