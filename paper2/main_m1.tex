...
and the exact-law peak predictions, registered before the refined
Monte-Carlo sweep completed, were confirmed out-of-sample at both fresh
points ($3\%$ and $6\%$ at $c_v = 0.02$ and $0.2$), with a pooled two-run
continuous-peak fit measuring the jitter exponent at $-0.658$ ($1.3\%$ from
$-2/3$). We report a preregistered simulation campaign (M1) with a single
...
% (docs/R14_PREREGISTERED_PREDICTIONS.md, commit 8a627bb) and CONFIRMED
% out-of-sample by the sweep (verdict commit 1d8d7aa; both fresh points hit,
% 3% and 6%); pooled zoom+Colab exponent -0.658, 1.3% from -2/3 (4d8310d,
% bd731d5); k=4/3 intermediate hypothesis preregistered 5e51088, refuted
% 1a32758.
...
% ---- Three-stage measurement vs. registered exact-law predictions ----------
% Sources of record: grid = results/round63_study2/jitter_sfi_v2_nu2000.log;
% zoom (local 150k) + Colab replication (400k) + pooled fit = the appended
% sections of docs/R14_PREREGISTERED_PREDICTIONS.md (commits 4d8310d,
% bd731d5). Predictions registered PRE-DATA at 8a627bb; grid verdict 1d8d7aa;
% k=4/3 intermediate hypothesis preregistered 5e51088, REFUTED 1a32758.
\begin{table}[t]
\centering
...
The exact-law column of Table~\ref{tab:jitter}, including the two
...
($6\%$) --- with every in-sample peak within one notch of the $\times1.157$
grid (verdict at commit \texttt{1d8d7aa}), and the $\nu = 200$ column is
consistent including the crossover correction of Eq.~(\ref{eq:crossover}).
The continuous-peak zoom and its independent replication then measured the
exponent itself:\ the pooled eight-measurement log-log fit of $\rho_{*}$
against $c_v$ gives slope $-0.658$, within $1.3\%$ of the predicted $-2/3$,
with the constant consistent with $2^{-1/3}$ within $5$--$10\%$.
Section~\ref{sec:results} reports the full three-stage verdict.
...
and $0.3$, respectively (Table~\ref{tab:jitter}); both predictions
...
$(9.645,\ 5.154,\ 3.752,\ 2.106)$ and replicated
$(9.610,\ 5.700,\ 3.399,\ 2.051)$ at $c_v = (0.02,\ 0.05,\ 0.1,\ 0.2)$,
respectively. The pooled eight-measurement log-log fit gives slope $-0.658$
