...
points ($3\%$ and $6\%$ at $c_v = 0.02$ and $0.2$). We report a preregistered simulation campaign (M1) with a single
...
% ---- Refined sweep vs. registered exact-law predictions --------------------
% Source of record: results/round63_study2/jitter_sfi_v2_nu2000.log (+ nu200);
% predictions registered PRE-DATA at commit 8a627bb, verdict at 1d8d7aa.
\begin{table}[t]
\centering
\caption{Refined-sweep scalar count information $J(\rho,\nu=2000,c_v)$ for
lognormal hold times (\texttt{jitter\_sfi\_v2}:\ $100{,}000$ frames per
cell, common random numbers, 25-point log grid of geometric spacing
$1.157$), against the exact-law peak predictions of
Theorem~\ref{thm:cubic}/Eq.~(\ref{eq:crossover}) registered \emph{before}
the sweep completed (\texttt{docs/R14\_PREREGISTERED\_PREDICTIONS.md},
commit \texttt{8a627bb}; verdict at commit \texttt{1d8d7aa}). Both
out-of-sample rows hit ($c_v = 0.02$:\ $3\%$; $c_v = 0.2$:\ $6\%$), and
every in-sample peak lies within one grid notch of its prediction. The
earlier coarse-grid fit $c_0 \approx 0.72$ was an effective finite-grid
coefficient; the exact asymptotic coefficient is $2^{-1/3} = 0.794$.
Hardware warning:\ at the deterministic ridge ($\rho = 22.3$), $5\%$ jitter
leaves $J = 0.43$ vs.\ $0.95$ --- about $55\%$ of the scalar count
information forfeited.
Source:\ \texttt{results/round63\_study2/}\allowbreak
\texttt{jitter\_sfi\_v2\_nu2000.log}.}
\label{tab:jitter}
\small
...
\hline
$0$    & $22.297$ & $22.25$ (exact ridge)          & $0.9452$ \\
$0.02$ & $10.739$ & $\approx10.4$ (out-of-sample)  & $0.8731$ \\
$0.05$ & $5.173$  & $5.69$                         & $0.7848$ \\
$0.1$  & $3.862$  & $3.50$                         & $0.6989$ \\
$0.2$  & $2.153$  & $\approx2.30$ (out-of-sample)  & $0.5882$ \\
...
out-of-sample rows, was registered before the refined sweep completed
(commit \texttt{8a627bb}); the sweep then confirmed the law at both
...
grid (verdict at commit \texttt{1d8d7aa}). The $\nu = 200$ column is
consistent including the crossover correction of
Eq.~(\ref{eq:crossover}):\ the $c_v = 0$ peak sits at $9.28$ against the
exact ridge $10.04$, and $c_v = 0.3$ measures $1.86$ against the predicted
$1.77$. Section~\ref{sec:results} restates the verdict with the campaign
results.
...
against the predicted $1.77$. The fuller three-hold-family exponent battery
...
and confirmed by the sweep (verdict at commit \texttt{1d8d7aa}).
